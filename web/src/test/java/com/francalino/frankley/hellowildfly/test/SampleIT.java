package com.francalino.frankley.hellowildfly.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "hello-wildfly-ear.ear");

        // Current directory is the root of the "hello-wildfly-web" project. Go up one level
        // and enter the "ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../ejb/target/hello-wildfly-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "hello-wildfly-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/hello-wildfly-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "hello-wildfly-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("com.francalino.frankley.hellowildfly.test");
